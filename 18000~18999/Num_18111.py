...
ground = [0 for i in range(257)]
tempMinValue = []
tempMaxValue = []

for i in range(in0[0]):
    temp = list(map(int, inputFast().split()))
    for j in temp:
        ground[j] = ground[j] + 1

    tempMinValue.append(min(temp))
    tempMaxValue.append(max(temp))

minValue = min(tempMinValue)
maxValue = max(tempMaxValue)
...
for h in range(minValue, maxValue+1):
    price = 0
    block = in0[2]
    for g in range(minValue, maxValue+1):
        if (h-g) > 0:
            price = price + (h-g)*ground[g]
            block = block - (h-g)*ground[g]
        elif h == g:
            pass
        else:
            price = price + (g-h)*2*ground[g]
            block = block + (g-h)*ground[g]

    if block >= 0:
        resultPrice.append(price)
        result.append([price, h])
    else:
        pass
# print(result)
...
for i in range(len(result)):
    if minPrice == result[i][0]:
        maxHeight.append(result[i][1])

print(minPrice, max(maxHeight))