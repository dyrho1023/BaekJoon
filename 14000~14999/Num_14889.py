...
def calDifference(start, link):
    sumStart = 0
    sumLink = 0
    for m in range(len(link)):
        for n in range(len(link)):
            sumStart += in1[array[m]][array[n]]
            sumLink += in1[link[m]][link[n]]

    difference = abs(sumStart - sumLink)
    return difference


def valancingTeam(i, array, numMax, starting):
    global tooSlow
    if i == numMax//2:
        linkList = []
        ...
        temp = calDifference(array, linkList)
        tooSlow = min(tooSlow, temp)
        # print(array, linkList)
        return

    for j in range(starting, numMax):
        if j not in array:
            array[i] = j
            valancingTeam(i+1, array, numMax, j+1)
            array[i] = 0


in0 = int(inputFast())
in1 = [list(map(int, input().split())) for _ in range(in0)]

array = [0 for i in range(in0//2)]
starting = 0
tooSlow = int(10000)

valancingTeam(0, array, in0, starting)
