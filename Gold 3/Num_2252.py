...
# in1 = [5, 4]
#
# inputOrder = [[1, 2], [3, 4], [5, 1], [4, 2]]
# inputOrder = [[0, 1], [2, 3], [4, 0], [3, 1]]
...
countList = [0 for _ in range(in1[0]+1)]
linkList = [[] for _ in range(in1[0]+1)]

for i in range(in1[1]):
    temp = list(map(int, inputFast().split()))
    inputOrder.append(temp)

for i, j in inputOrder:
    countList[j] += 1
    linkList[i].append(j)

for i in range(1, in1[0]+1):
    if (countList[i] == 0):
        checkQueue.append(i)

while(checkQueue):
    check = checkQueue.popleft()
    for i in linkList[check]:
        countList[i] -= 1
