...
begin, begin+1, ..., mid-1, mid, mid+1, ..., end-1, end 순서로 되어있는 바구니의
순서를 mid, mid+1, ..., end-1, end, begin, begin+1, ..., mid-1로 바꾸게 된다.
...
ball = [i+1 for i in range(in0[0])]

for i in range(in0[1]):
    [temp0, temp1, temp2] = list(map(int, input().split()))
    check = deque(*[ball[temp0-1:temp1]])
    check.rotate(temp1-temp2+1)
    ball[temp0-1:temp1] = check
