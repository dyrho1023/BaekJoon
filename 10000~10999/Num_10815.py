...
in3 = int(inputFast())
in4 = list(map(int, inputFast().split()))
...
def BinarySearch(target, inputList, start, end):
    ...
    if inputList[half] > target:
        return BinarySearch(target, inputList, start, half - 1)
    elif inputList[half] < target:
        return BinarySearch(target, inputList, half + 1, end)
    else:
        return 1
...
for i in in4:
    result.append(BinarySearch(i, in2, 0, in1-1))
