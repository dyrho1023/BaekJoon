...
def checkGrid (paper):
    ...
    for i in range(len(paper)):
        sumPaper = sumPaper + sum(paper[i])

    if sumPaper == 0:
        count_white = count_white + 1
    elif sumPaper == size**2:
        count_blue = count_blue + 1
    else:
        if halfSize > 0:
            paper_1 = paper[:halfSize]
            paper_2 = paper[:halfSize]
            paper_3 = paper[halfSize:]
            paper_4 = paper[halfSize:]

            for i in range(len(paper_1)):
                paper_1[i] = paper_1[i][:halfSize]
                paper_2[i] = paper_2[i][halfSize:]
                paper_3[i] = paper_3[i][:halfSize]
                paper_4[i] = paper_4[i][halfSize:]
