...
for i in range(in0):
    ...
    document.append([temp0, temp1])
    prior.append(list(map(int, inputFast().split())))


for i in range(in0):
    location = document[i][1]
    priorQueue = collections.deque(prior[i])
    count = 0
    while(True):
        maxNum = max(priorQueue)
        currentDocx = priorQueue.popleft()
        if maxNum == currentDocx:
            count += 1
            if location == 0:
                print(count)
                break
            ...
            priorQueue.append(currentDocx)
            if location == 0:
                location = len(priorQueue)-1
            else:
                location = location - 1
