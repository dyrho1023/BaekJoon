...
서브태스크
1. 모든 주유소의 리터당 가격은 1원이다. (17점)
2. 2 ≤ N ≤ 1,000, 제일 왼쪽 도시부터 제일 오른쪽 도시까지의 거리는 최대 10,000, 리터 당 가격은 최대 10,000이다. (41주)
3. 원래의 제약조건 이외에 아무 제약조건이 없다. (42주)
"""
...
minPrice = in3[0]
totalPrice = in3[0]*in2[0]

for i in range(1, in1-1):
    if minPrice < in3[i]:
        totalPrice += in2[i]*minPrice
    else:
        minPrice = in3[i]
        totalPrice += in2[i]*minPrice
